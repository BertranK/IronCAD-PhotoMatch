...
## Document isolation, window restore and pixel diagnostics (2026-09-14)

This checkpoint supersedes the latest GUI-workflow and capture-recovery blockers above. Proto 0 is still incomplete; the one-physical-pixel photo alignment gate has not been relaxed.

- Installed native DLL SHA-256: `090FCD8246176BD8ABAAC0FCFD0093363067ED1413E5233727C2F25FF0DFBBBF`; SDK `29.0.2.20605`. Windows is restored to 150%, with fresh host DPI 144. No Windows theme change was made in this checkpoint.
- Switching from the generated fixture to the LEGO document exposed stale point rows after camera restoration. Context teardown now clears old point references, measurements and restore diagnostics. `context-fix-document-switch-cleared.json` and `context-fix-gui-empty.jpg` show an empty new-document state. Opening the saved user result then reconnected all six verified model references (`context-fix-six-points-reconnected.json`).
- The installed DLL passed the 16-corner fixture and two-pose/full-camera-restore check (`context-fix-coordinate-roundtrip.json`). The prior DLL also passed fresh 100% portrait/landscape analytical runs. Screen capture recovered for the fresh 150% portrait/landscape runs; `analytical-api-150-native-portrait.jpg` and `analytical-api-150-native-landscape.jpg` show the reference photo and model. These displayed captures are scaled; they are qualitative evidence, not independent physical-pixel measurements.
- Analytical continuous-photo versus integer-SDK maximum distances: 100% landscape 1.2217 px, portrait 1.3495 px; fresh 150% landscape 1.2911 px, portrait 1.3495 px. A host started at 100% and then virtualized at 150% gave 2.0742 px. These remain failed. Matching both operands to the SDK integer grid gives 1.0 px in the native-DPI cases and 1.4998 px in the virtualized case. The new `sdk_raster_max_error_px` / `sdk_raster_passed` fields are diagnostics only. `screen_max_error_px` / `screen_passed` retain the continuous comparison and the original <= 1 physical-pixel gate. Some earlier `analytical-api-*` files were recorded before this field separation; their old `screen_passed` describes the raster diagnostic, not final acceptance. `context-fix-analytical-preview.json` uses the final names and correctly records screen failure.
- Minimize/restore reproduced a blank PhotoMatch window. Native diagnostics showed unchanged managed child bounds, but its actual screen origin moved from (87,121) to (32174,32242). Visibility toggling alone failed. The project now checks the embedded child position after native move/resize events and reapplies its managed bounds only when displaced, on the UI thread, without activating or reordering windows. It uses the documented child-client coordinates of [SetWindowPos](https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-setwindowpos). No dependency files were modified.
- Actual GUI verification at 150% passed: minimize -> existing-window launch reuse; Open results -> minimize/restore -> Calculate -> Preview -> Restore -> Save; maximize -> normal size. Evidence: `gui-minimize-blank-before.jpg`, `gui-minimize-photo-restored.jpg`, `gui-bounds-fix-maximized.jpg`, `gui-bounds-fix-preview.json`, and `gui-bounds-fix-restored.json`. Full camera values and model transforms/bounds matched after Restore, with six points retained and no host errors.
- `sky-blue-bounds-fix-verified.json` was saved through the actual GUI. Its correspondences and complete model point records exactly equal `sky-blue-user-edited.json`. Maximum residual remains 3.364254 original-image pixels; screen discrepancy remains 8.880403 physical pixels. Both acceptance flags are false. The original edited result and sample model were not overwritten.
- Validation: 32 Python tests and seven browser tests passed after the changes, including a regression proving that raster agreement cannot mask a failed continuous comparison. Native v143 build and projection tests passed before installing the document-isolation change.

Remaining: resolve and verify rendered photo placement versus the SDK integer projection, complete the final-build 100%/150% visual matrix, and address the sample's residual without altering the user's points to manufacture a pass. General-user installer packaging and lens calibration remain outside this delivered checkpoint.


## Actual draw bounds and floating projection diagnosis (2026-09-14)

- Installed diagnostic DLL SHA-256: `E51CE97E106F7C6C1ED4A17119F46C84FBDC57771DA06C694631B844E8BD09AB`. Native build and projection tests passed. GUI suite: 33 Python tests and seven browser tests passed.
- The overlay reports `photo_drawn_rectangle_physical` only after committing the backing frame. The measurement uses these actual rounded draw bounds, separately retaining the ideal rectangle. A rectangle change now invalidates the backing image even if the window extent is unchanged. `screen_image_rectangle_verified` distinguishes the updated renderer from older hosts.
- `drawn-bounds-analytical-150-landscape.json` records actual bounds (196,0,2947,1842), ideal bounds (195.9,0,2947.2,1842), and integer-SDK-to-photo maximum 1.302241 physical pixels. It remains failed; correcting the measurement bounds alone did not resolve the discrepancy.
- The installed build passed the 16-corner/two-camera/full-restore fixture (`inverse-world-coordinate-roundtrip.json`). It also records model-to-view floating coordinates for selected points, strictly as diagnostics; these are not used by the acceptance gate.
- In `model-double-analytical-150-landscape.json`, those floating coordinates differ from the actual drawn analytical-photo positions by at most 0.072337 physical pixels. The same frame's world integer comparison is 1.302241 pixels. Existing independent camera-relative probes across native 100%, native 150% and virtualized 150% differ from the analytical floating formula by less than 0.0005 physical pixel (`continuous-projection-diagnostic-analysis.json`). This implicates integer-versus-continuous coordinate comparison, but does not yet prove the model-to-view API is a valid world-space replacement in every render frame.
- An inverse-world/model-forward probe at input depths 0 and 1 did not round trip consistently. Those depth inputs have not been established as the SDK's valid depth convention. These samples are retained as diagnostics, not a validation or an API-failure verdict. No rendering matrix was changed.
- Full restoration passed after this run (`model-double-analytical-restored.json`); the open scene contains only generated fixture geometry. User photos, edited point files, and saved sample models remain unchanged.

Next verification: establish a guarded floating world-projection path using independent noncoplanar world anchors spanning the selected points, compare it against both the known camera formula and SDK integer world output in each frame, then repeat the actual-draw-bounds photo matrix. Do not turn diagnostic floating agreement into a pass before validating the coordinate frame.
