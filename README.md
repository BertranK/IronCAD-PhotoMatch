...
현재 PnP 계산과 렌즈 보정은 포함하지 않는다. Proto 0의 전체 수치·화면 시험은 아직 미완료다.
...
상단 언어 선택은 시스템 / 한국어 / English를 제공하며 기본값은 시스템이다.
...
3. 대응점 행 선택 → 사진 클릭. 확대·이동·창 크기에 관계없이 원본 이미지 픽셀로 저장한다.
4. 카메라 조정/진단에서 테스트 카메라 적용, 투영 측정, 복원을 실행한다.
5. 화각 규약을 판별한 뒤 IronCAD에서 확인 버튼으로 사진 오버레이를 시험한다.
6. 결과 저장으로 사진 정보·대응점·호스트 검증 결과를 JSON에 저장한다.
